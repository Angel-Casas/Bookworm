...
// The Search-this-book button is gated by api-key + selected-model state
// (mirrors the Ask AI gate from Phase 4.4). With no key configured (the
// default fixture state) the toggle is hidden — the chat panel renders the
// no-key empty state instead of the composer + toggle.
test('Search button is hidden when no API key is configured', async ({ page }) => {
  await page.goto('/');
  await importFixture(page);
  await openImportedBook(page);

  await expect(page.getByLabel(/search this book/i)).toHaveCount(0);
});
