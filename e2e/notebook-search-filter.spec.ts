...
test('search + filter chips narrow the notebook list', async ({ page }) => {
  ...
  // Search a unique-to-note word (not present in the highlighted text).
  await page.getByRole('searchbox').fill('analysis');
  await page.waitForTimeout(250);
  await expect(page.locator('.notebook-row')).toHaveCount(1);
  await expect(page.locator('.notebook-row__note-line')).toContainText('analysis');
